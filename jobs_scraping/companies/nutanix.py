...
class NutanixApiJobScrapper(APIJobScrapper):
    ...
    def get_jobs(self):
        url = 'https://nutanix.eightfold.ai/api/apply/v2/jobs?num=10'
        offset = 0
        postings = []
        while True:
            ...
            for doc in data:
                apply_url = 'https://nutanix.eightfold.ai/careers?pid=' + str(doc['id'])
                postings.append({"title": doc['name'], "description": doc['job_description'], "location": doc['location'], "url": apply_url})
