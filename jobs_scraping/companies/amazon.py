...
class AmazonApiJobScrapper(APIJobScrapper):
    ...
    def get_jobs(self):
        url = 'https://www.amazon.jobs/en-gb/search.json?result_limit=100'
        offset = 0
        postings = []
        while True:
            ...
            if not data:
                break
            for doc in data:
                postings.append({"title": doc['title'], "description": doc['description'], "location": doc['location'], "url": doc['url_next_step']})
